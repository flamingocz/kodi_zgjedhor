# Content review

Reviewed against the supplied folder on 6 September 2026. This is an editorial implementation record, not a legal opinion or a statement of the initiative’s current legislative status.

The Albanian, English, and bilingual leaflets now show the updated signature deadline of 15 December 2026. The website’s Albanian and English deadline notices match the updated leaflets.

The supplied `reformazgjedhore-qr.svg` is embedded in the lower-right whitespace of each leaflet page and points to the website. A second high-resolution QR code points to the Veprimi Qytetar Facebook page and is placed beside it.

The form-filling guidance was adapted from the linked practical diaspora guide supplied during implementation. That guide concerns a different referendum, so only the general handling rules were applied here: complete the designated fields from a valid Albanian identity document, use blue capital letters, verify the data, do not copy or photograph the ID, and sign personally. Its unrelated referendum title and dates were not copied. Per the current campaign instruction, a clear scan or photograph of the physically signed form may be emailed to `veprimi.qytetar@gmail.com`.

## Source precedence

1. `Amendime Kodi zgjedhor-Veprimi Qytetar.docx` for amendment substance and article references.
2. `Formulari-20mije-Firmat.pdf` for the actual signature sheet and its fields.
3. The user’s latest confirmed instructions for this campaign: sign physically, then choose a local collector, direct post, or email a scan to `veprimi.qytetar@gmail.com`. This restores the third route in `permbledhje-chat.md` and supersedes the earlier original-paper-only instructions.
4. The summaries and leaflets for accessible explanations; the English overview draws on the English leaflet, with participation guidance translated from the confirmed instructions.
5. Media coverage for further reading only. The Euronews link was readable; the Lapsi article could not be retrieved by the browsing tool. Its supplied link is retained without deriving factual claims from it.

## Items requiring organizer/editor clarification

- **Collection period:** the earlier leaflet version stated three months from 15 June to 15 September 2026. The local request for KQZ forms says two months. The Euronews report describes a three-month request. The official signature sheet identifies a model approved by decision 37 of 15 June 2026 but does not establish the collection deadline. The current leaflet and website version now show 15 December 2026 as the updated signature deadline. The requirement of at least 20,000 signatures comes from the supplied leaflets and is distinguished from a collected-signature count. Readers are directed to the team for delivery arrangements.
- **Identification abroad:** the signature form’s column is “Numri ID”; the chat instructions refer to an identity-card serial number in Albania and an Albanian passport number abroad. The page lists the actual form fields and directs overseas identification questions to Edmond instead of guessing which identifier to enter.
- **Territorial list:** draft article 42 includes “Pukë” in a list described as counties and does not mention Lezhë. The website lists the four region names only, flags the territorial list as needing clarification, and does not invent a map or silently correct the draft.
- **Numbering and incomplete wording:** the provision adding Code article 113/1, positioned between draft articles 66 and 68, is labeled “Neni 7” in the original. Other paragraphs are repeated or incomplete (including articles 13, 28, 49, 72 and 82). The counting summary cites unambiguous articles 66, 68–72 and related changes instead of silently relabeling the repeated 7 as 67.
- **Gender wording:** draft article 37’s initial municipal-list wording is unclear; articles 79 and 81 provide explicit rules for reranking. Public explanations use those explicit reranking rules.
- **Seat allocation:** draft article 78 specifies odd-number divisors and includes rounding/termination wording that should be clarified before implementing any seat calculator. The website describes only the proposed sequence and regional allocation; it makes no numerical seat predictions.
- **Media allocation:** articles 44–48 contain detailed channel and airtime arrangements whose wording should be reviewed by the authors. The website summarizes the proposed access, transparency and enforcement mechanisms without inventing an operational broadcast schedule.
- **Representative committee:** the local KQZ request lists five people, whereas the leaflets and Euronews list ten. The website attributes source materials to Veprimi Qytetar without publishing a reconciled committee roster or the personal ID numbers in the request.

## Confirmed and implemented

- Signature form: `Formulari-20mije-Firmat.pdf`, two pages. Required columns verified from extracted text and a rendered first page: name, surname, father’s name, date of birth, birthplace, ID number, contact number, signature.
- Coordination: Edmond Cata, +355 694382248, edmondcata@hotmail.com; telephone, WhatsApp and email links.
- Three delivery routes are displayed in both languages: local collector, direct post, or an emailed scan of the physically signed form. The team supplies postal arrangements; no postal address is invented. Email links open the visitor’s email application; the website itself does not upload or send documents.
- Volunteer participation through direct contact; no form/backend, messages sent, tracking, or uploaded identity documents.
- All proposal explanations are framed as proposed provisions and aims, not as enacted law or guaranteed outcomes.
- The summary’s broader description of financial-office staff qualifications is not repeated: the draft specifies economics/finance for employees and also permits law for the director.
- The separate protected-areas referendum document is excluded.
- The supplied leaflets were updated visually to show the 15 December 2026 deadline; the draft, summary, and official form remain unchanged.
